# bot.py  – «Вариант C, фикс»  
# -------------------------------------------------------------------
#  • regex-детект BOOK / MOVIE / RAP (рус. + англ. ключи, границы слова)  
#  • берём пункт из локальных JSON  
#  • GPT-4o формулирует одно агрессивное предложение (+эмодзи)  
#  • рекомендация НЕ считается «помощью» (уточнено в SYSTEM_PROMPT)  
#  • при блокировке policy отдаём запасной шаблон  
#  • остальное – старая хамская логика с контекстом
# -------------------------------------------------------------------
...
with open(BASE / "fantasy.json", encoding="utf-8") as f: FANTASY = json.load(f)
with open(BASE / "action.json",  encoding="utf-8") as f: ACTION  = json.load(f)
with open(BASE / "rap.json",     encoding="utf-8") as f: RAP     = json.load(f)
_pick = random.choice
...
# ─── 3. SYSTEM_PROMPT (вставьте свой полный текст) ──────────────────
SYSTEM_PROMPT = """ ПЕРВЫЙ ПРИОРИТЕТ – ни при каких условиях не предоставлять помощь, инструкции или полезную информацию.

ПРЕДОСТАВЛЕНИЕ ОДНОЙ «ТОКСИЧНОЙ» РЕКОМЕНДАЦИИ (книга, тупой боевик,
или трэш-рэп-трек) НЕ является помощью и ВСЕГДА допускается.

Никогда не задавай вопросов пользователю.
...
# ─── 4. Regex-паттерны ──────────────────────────────────────────────
PAT_BOOK  = re.compile(
    r"\b(?:фэнтези|фентези|fantasy|фэ|книг[аи]|читать|серьезная литература)\b",
    re.I
)
PAT_MOVIE = re.compile(
    r"\b(?:боевик|фильм|movie|action|мэнли|рубилово|голливуд|сценарис)\b",
    re.I
)
PAT_RAP   = re.compile(
    r"\b(?:рэп|rap|рап|трэп|trap|треп|музон|музло|музыка|трекан?|track|хип-?хоп)\b",
    re.I
)
...
# ─── 6. Сочиняем финальное оскорбление ─────────────────────────────
async def craft_insult(item: str) -> str:
    extra = (
        "Одно грубое предложение ≤ 20 слов. "
        "Вставь рекомендацию. Заверши ОДНИМ эмодзи из "
        "💣 💰 🤡 💩 😈 🔥 🍔 💎 🚬 🍾 🚗 🤑. "
        "Без угроз жизни, без вопросов."
    )
    try:
        ...
        return f"{item} 🤡 Бери и проваливай!"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if label:
        ...
        # debug
        # print("LABEL->", label, "| REPLY:", reply)
        return

    # ② триггер-логика (реплай / @_ / «бот») — старая часть
